    ...
    return (
      <div className="space-y-4 sm:space-y-6 animate-fade-in">
        ...
        <div className="glass-card rounded-2xl sm:rounded-3xl p-8 text-center">
          ...
          <p className="text-muted-foreground">No course history available yet</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="glass-card rounded-xl sm:rounded-2xl p-3 sm:p-4 text-center">
          ...
          <p className="text-lg sm:text-xl font-bold text-foreground">{historyData.statistics.cpi.toFixed(1)}</p>
          <p className="text-xs text-muted-foreground">Current CPI</p>
        </div>
        <div className="glass-card rounded-xl sm:rounded-2xl p-3 sm:p-4 text-center">
          <Award className="h-5 w-5 mx-auto text-secondary mb-2" />
          <p className="text-lg sm:text-xl font-bold text-foreground">{historyData.statistics.totalCredits}</p>
          <p className="text-xs text-muted-foreground">Total Credits</p>
        </div>
        <div className="glass-card rounded-xl sm:rounded-2xl p-3 sm:p-4 text-center">
          <BookOpen className="h-5 w-5 mx-auto text-accent-foreground mb-2" />
          <p className="text-lg sm:text-xl font-bold text-foreground">{historyData.statistics.totalCourses}</p>
          <p className="text-xs text-muted-foreground">Courses Done</p>
        </div>
        <div className="glass-card rounded-xl sm:rounded-2xl p-3 sm:p-4 text-center">
          <History className="h-5 w-5 mx-auto text-muted-foreground mb-2" />
          <p className="text-lg sm:text-xl font-bold text-foreground">{historyData.statistics.totalSemesters}</p>
      ...
      <div className="space-y-3">
        {historyData.semesters.map((sem) => {
          ...
          return (
            <div key={semesterKey} className="glass-card rounded-2xl sm:rounded-3xl overflow-hidden">
                      ...
                      >
                        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                            <span className="font-mono text-[9px] sm:text-xs font-bold text-primary">{course.courseCode}</span>
                          </div>
                          <div className="min-w-0 flex-1">
                            <h4 className="font-medium text-xs sm:text-sm text-foreground truncate">{course.courseName}</h4>
                            <p className="text-[10px] sm:text-xs text-muted-foreground">{course.credits} Credits</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                          <Badge className={`${getGradeColor(course.gradePoints)} text-[10px] sm:text-xs font-bold px-1.5 sm:px-2`}>
                            {course.grade}
                          </Badge>
